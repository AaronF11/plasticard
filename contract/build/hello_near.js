...
let CreditContract = (_dec = NearBindgen({}), _dec2 = initialize(), _dec3 = view(), _dec4 = view(), _dec5 = view(), _dec6 = call({}), _dec(_class = (_class2 = class CreditContract {
  ...
  // This method is read-only and can be called for free
  get_example_method() {
    // This method give us an array of example commands to use a call method
    const array = ['get_example >>> Obtienes un ejemplo de la estructura <<<', 'get_example_method >>> Obtienes un arreglo de métodos disponibles <<<', 'set_user >>> Manda un usuario con todos sus atributos'];
    const examples = [];
...
function set_user() {
  ...
  const _result = _contract.set_user(_args);
...
function get_example_method() {
  ...
  const _args = CreditContract._getArgs();
  const _result = _contract.get_example_method(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CreditContract._serialize(_result, true));
}
function get_example() {
  ...
  const _args = CreditContract._getArgs();
  const _result = _contract.get_example(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(CreditContract._serialize(_result, true));
}
function init() {
  const _state = CreditContract._getState();
